 ...
 return (
  <PublicLayout>
   ...
   <div className='container mt-4'>
    <h3 className='mb-4'><i className='fas fa-map-marker-alt text-primary'></i>Track Your Order</h3>
    <div className='input-group mb-3 shadow-sm'>
     <span className='input-group-text bg-white'><i className='fas fa-receipt text-muted'></i></span>
    ...
    <button onClick={() => handleTrack(orderNumber)} className='btn btn-primary mb-4'><i className='fas fa-truck me-1'></i> Track</button>
    ...
    {trackingData.length > 0 && (
     <div className='card p-4 shadow-sm rounded-4 border-0'>
      ...
      <div className='d-flex justify-content-evenly align-item-center mb-5 px-2 position-relative '>
       ...
       {trackingData.map((entry, index) => (
        <div key={index} className='text-center flex-fill timeline-step'>
         ...
         <small className='fw-bold d-block'>{entry.status}</small>
         <small className='fw-muted'>{new Date(entry.status_date).toLocaleString()}</small>
        </div>
       ))}
      </div>
