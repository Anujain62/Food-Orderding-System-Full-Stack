      ...
      <div className='collapse navbar-collapse'>
        <ul className='navbar-nav ms-auto align-item-center gap-3'>
          <li className='nav-item'>
            <button onClick={() => { if (newOrders > 0) navigate('/order-not-confirmed') }} title={newOrders > 0 ? 'View New Orders' : 'No New Orders'} className='btn btn-outline-secondary position-relative'>
              <FaBell />
              <span className='badge bg-danger position-absolute top-6 start-2'>{newOrders}</span>
            </button>
          </li>
          <li className='nav-item'>
            <button className='btn btn-outline-danger' onClick={handleLogout}> <FaSignOutAlt className='me-1' /> Logout </button>
          </li>
        </ul>
      </div>
